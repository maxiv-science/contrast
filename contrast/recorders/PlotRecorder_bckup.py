from . import Recorder
from ..environment import macro

import signal
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.lines import Line2D
import matplotlib.cm as cm
import matplotlib.animation


def dict_lookup(dct, path):
    """
    Helper to recursively get dct['path']['to']['item'] from
    dct['path/to/item'].
    """
    if '/' in path:
        pre, post = path.split('/', maxsplit=1)
        return dict_lookup(dct[pre], post)
    else:
        return dct[path]

class PlotRecorder(Recorder):
    """
    Recorder which catches data and plots it with matplotlib.

    Unlike the base class Recorder, the GUI event loop takes care of
    the timing (plt.timer) and when to close (plt.show).
    """

    def __init__(self, data1, data2=None, name='plot'):
        Recorder.__init__(self, name=name)
        if data2 is not None:
            self.xdata = data1
            self.ydata = data2
        else:
            self.xdata = None
            self.ydata = data1
        self.nplots = 0

    def init(self):
        # set up figure and axes
        self.fig = plt.figure()
        self.fig.canvas.set_window_title(self.name)
        self.ax = self.fig.gca()
        self.ax.set_xlabel(self.xdata)
        self.ax.set_ylabel(self.ydata)

        # add a timer to trigger periodic checking of the queue
        self.timer = self.fig.canvas.new_timer(interval=int(self.delay*1000))
        self.timer.add_callback(self._timer_callback)
        self.timer.start()

        # blocking show() manages when the application should close
        plt.show()

    def _timer_callback(self):
        # this SIGINT handling has to be set up here, for some reason
        signal.signal(signal.SIGINT, signal.SIG_IGN)
        self._process_queue()
        self.periodic_check()
        if self.quit:
            self._close()

    def run(self):
        self.init()

    def act_on_header(self, dct):
        # start a new scan
        self.nplots += 1
        self.x = []
        self.new_scan = True
        self.scannr = dct['scannr']

    def act_on_data(self, dct):
        # if our data isn't in dct, just move on
        try:
            new_data = dict_lookup(dct, self.ydata)
        except KeyError:
            return

        # be ready to get data that are dicts instead of numbers,
        # so may as well just work with dicts.
        if not isinstance(new_data, dict):
            new_data = {'': new_data}

        # we can only set up lines etc once we know what is actually
        # in the data.
        if self.new_scan:
            self.new_scan = False
            col = 'bkmrcg'[(self.nplots-1) % 6]
            styles = {k:['solid', 'dashed', 'dotted', 'dashdot'][i%4] for i, k in enumerate(new_data.keys())}
            self.lines = {key: Line2D(xdata=[], ydata=[], color=col, linestyle=styles[key], label='%d: %s'%(self.scannr, key)) for key in new_data.keys()}
            self.y = {key: [] for key in new_data.keys()}
            for k, l in self.lines.items():
                self.ax.add_line(l)
            self.ax.legend()

        # ok treat the actual data
        for k, v in new_data.items():
            self.y[k].append(v)
        if self.xdata is not None:
            # checking if we have explicit x data
            self.x.append(dct[self.xdata])
        else:
            self.x = range(len(self.y[k]))
        for k, l in self.lines.items():
            l.set_data(self.x, self.y[k])

        self.ax.relim()
        self.ax.autoscale_view()
        plt.draw()

    def _close(self):
        plt.close(self.fig)

class PlotRecorderMesh(Recorder):
    """
    Recorder which catches data and plots it with matplotlib.

    Unlike the base class Recorder, the GUI event loop takes care of
    the timing (plt.timer) and when to close (plt.show).
    """

    def __init__(self, data1, data2, data_z, name='plot'):
        Recorder.__init__(self, name=name)
        self.xdata = data1
        self.ydata = data2
        self.zdata = data_z
        self.nplots = 0

    def init(self):
        # set up figure and axes
        self.X, self.Y, self.Z = [],[],[]
        self._x, self._y, self._z = None, None, None
        self.new_point = False
        
        self.fig = plt.figure()
        self.fig.canvas.set_window_title(self.name)
        self.ax = self.fig.gca()
        self.ax.set_xlabel(self.xdata)
        self.ax.set_ylabel(self.ydata)
        self.sc = self.ax.scatter(self.X, self.Y, self.Z, marker="o", sizes=(100,), cmap=cm.Greys_r)
        self.anm = matplotlib.animation.FuncAnimation(self.fig, self._update_plot,
                                                        frames=None, interval=100, repeat=False)

        # add a timer to trigger periodic checking of the queue
        #self.timer = self.fig.canvas.new_timer(interval=int(self.delay*1000))
        #self.timer.add_callback(self._timer_callback)
        #self.timer.start()

        # blocking show() manages when the application should close
        plt.show()
        
        #ax.pcolormesh(xpos, ypos, z, cmap='gray', shading='auto', snap=True)
        #self.plt.xlabel("X, $\mu m$")
        #self.plt.ylabel("Y, $\mu m$")

    def _update_plot(self, arg):
            # this SIGINT handling has to be set up here, for some reason
        signal.signal(signal.SIGINT, signal.SIG_IGN)
        self._process_queue()
        self.periodic_check()
        #print(f'#### _update_plot call: self.new_point: {self.new_point}')
        if self.new_point:
            self.X.append(np.random.rand(1)*10)
            self.Y.append(np.random.rand(1)*10)
            self.Z.append(np.random.rand(1)*10)
            # self.X.append(self._x)
            # self.Y.append(self._y)
            # self.Z.append(self._z)
            print(f'#### _update_plot call: arg, self.X, self.Y, self.Z are: {arg}, {self.X}, {self.Y}, {self.Z}')
        self.sc.set_offsets(np.c_[self.X, self.Y])
        self.new_point = False
        
        if self.quit:
            self._close()        

    def _timer_callback(self):
        # this SIGINT handling has to be set up here, for some reason
        signal.signal(signal.SIGINT, signal.SIG_IGN)
        self._process_queue()
        self.periodic_check()
        if self.quit:
            self._close()
    
    def _get_new_point(self, dct, xname='finex', yname='finey', zname='roi'):
        self.entries = list(dct.items())
        x_val = dct[xname]
        y_val = dct[yname]
        z_val = dct[zname]
        #print(f'The input dct in _get_new_point is {dct}')
        return (x_val, y_val, z_val)

    def run(self):
        self.init()

    def act_on_header(self, dct):
        # start a new scan
        self.scannr = dct['scannr']
        self.desc = dct['description'].split()
        self.xdata_param, self.ydata_param = self.desc[2:5], self.desc[6:9]
        self.ax.set_xlim(self.xdata_param[0], self.xdata_param[1])
        self.ax.set_ylim(self.ydata_param[0], self.ydata_param[1])

        print(f'#### act_on_header call: self.xdata_param, self.ydata_param are {self.xdata_param}, {self.ydata_param}')
        
        #self.xx, self.yy = np.meshgrid(self.X, self.Y)
        #self.Z = np.empty(shape=(int(self.xdata_param[2])+1, int(self.ydata_param[2])+1))
        #self.sc = self.ax.scatter(self.xx, self.xx, c=self.Z, marker="o", s=100, cmap=cm.Greys_r)
        
        # self.X = np.empty(shape=(int(self.xdata_param[2])+1, int(self.xdata_param[2])+1), dtype=np.float64)
        # self.Y = np.empty(shape=(int(self.ydata_param[2])+1, int(self.ydata_param[2])+1), dtype=np.float64)
        # self.Z = np.empty(shape=((int(self.xdata_param[2])+1), (int(self.ydata_param[2])+1)), dtype=np.float64)

        # self.fast_ax_max_i = int(self.ydata_param[2])+1
        # self.slow_ax_max_i = int(self.xdata_param[2])+1
        
        # X and Y arrays prepared for scatter
        # self.X = np.zeros(shape=(self.slow_ax_max_i ** 2, ), dtype=np.float64)
        # self.Y = np.zeros(shape=(self.fast_ax_max_i ** 2, ), dtype=np.float64)
        # self.Z = np.zeros(shape=(self.slow_ax_max_i * self.fast_ax_max_i, ), dtype=np.float64)

        # X and Y arrays prepared for pcolormesh
        # self.X = np.linspace(float(self.xdata_param[0]), float(self.xdata_param[1]), int(self.xdata_param[2])+1)
        # self.Y = np.linspace(float(self.ydata_param[0]), float(self.ydata_param[1]), int(self.ydata_param[2])+1)
        # self.Z = np.zeros(shape=(self.slow_ax_max_i, self.fast_ax_max_i), dtype=np.float64)

        # self.arr_i = 0
        # self.arr_x_i = 0
        # self.arr_y_i = 0
        
        #self.sc.set_offsets(np.c_[x,y,z])
        #self.sc = self.ax.scatter(self.X, self.Y, c=self.Z, marker="o", s=100, cmap=cm.Greys_r)
        #self.clr_mesh = self.ax.pcolormesh(self.X, self.Y, self.Z, cmap='gray', shading='auto', snap=True)
        #self.cb1 = self.ax.colorbar(clr_mesh)
        # plt.draw()
        # self.fig.canvas.flush_events()

    def act_on_data(self, dct):
        # be ready to get data that are dicts instead of numbers,
        # so may as well just work with dicts.
        # if not isinstance(new_data, dict):
        #     return
        self.new_point = True
        x, y, z = self._get_new_point(dct)
        self._x, self._y, self._z = x, y, z
        print(f'#### act_on_data call: self._x, self._y, self._z are {self._x}, {self._y}, {self._z}')
        print(f'#### act_on_data call: dct is: {dct}')

        #print(f'The z type is {type(z)}, value is {z}')
        
        # if self.fast_ax_i < int(self.ydata_param[2]):
        #     self.X[self.slow_ax_i, self.fast_ax_i] = float(x)
        #     self.Y[self.slow_ax_i, self.fast_ax_i] = float(y)
        #     self.Z[self.slow_ax_i, self.fast_ax_i] = float(z)
        #     self.fast_ax_i += 1
        # else:
        #     self.X[self.slow_ax_i, self.fast_ax_i] = float(x)
        #     self.Y[self.slow_ax_i, self.fast_ax_i] = float(y)
        #     self.Z[self.slow_ax_i, self.fast_ax_i] = float(z)
        #     self.fast_ax_i = 0
        #     self.slow_ax_i += 1
        '''
        max_indx = (int(self.ydata_param[2])+1)**2

        np.reshape(self.X, (9,))
        np.reshape(self.Y, (9,))
        np.reshape(self.Z, (9,))
        
        if self.arr_i <= max_indx:
            self.X[self.arr_i] = float(x)
            self.Y[self.arr_i] = float(y)
            self.Z[self.arr_i] = float(z)
            self.arr_i += 1

        np.reshape(self.X, (3,3))
        np.reshape(self.Y, (3,3))
        np.reshape(self.Z, (3,3))

        print(f'The index is: {self.arr_i}, the Z array is: {self.X.tolist()}')
        print(f'The index is: {self.arr_i}, the Z array is: {self.Y.tolist()}')
        print(f'The index is: {self.arr_i}, the Z array is: {self.Z.tolist()}')
        #print(self.Y.tolist())
        #print(self.Z.tolist())
        '''
        
        # N = 9
        # self.X = np.random.rand(N)
        # self.Y = np.random.rand(N)
        # self.Z = np.random.rand(N)
        #colors = np.random.rand(N)
        #area = (30 * np.random.rand(N))**2

        # if self.arr_i < (self.slow_ax_max_i * self.fast_ax_max_i):
        #     self.X[self.arr_i] = float(x)
        #     self.Y[self.arr_i] = float(y)
        #     self.Z[self.arr_i] = float(z)
        #     self.arr_i += 1
        # else:
        #     pass

        # x_arr = self.X.reshape((self.slow_ax_max_i, self.slow_ax_max_i)).copy()
        # y_arr = self.Y.reshape((self.fast_ax_max_i, self.fast_ax_max_i)).copy()
        # z_arr = self.Z.reshape((self.slow_ax_max_i, self.fast_ax_max_i)).copy()
        # z = np.random.uniform(0, 100)

        # if self.arr_y_i < self.fast_ax_max_i-1:
        #     self.Z[self.arr_x_i, self.arr_y_i] = z
        #     self.arr_y_i += 1
        # elif self.arr_y_i == self.fast_ax_max_i-1:
        #     self.Z[self.arr_x_i, self.arr_y_i] = z
        #     self.arr_x_i += 1
        #     self.arr_y_i = 0

        # print(f'Shape X is {np.shape(self.X)}, Y is {np.shape(self.Y)}, Z is {np.shape(self.Z)}')
        # print(f'{self.X}')
        # print(f'{self.Y}')
        # print(f'{self.Z}')

        # x = np.array([-5.,  0.,  5.])
        # y = np.array([-5.,  0.,  5.])
        # z = np.array([[32.03600751,  6.71329204, 64.28788712],
        #                 [9.74235672, 38.22769079, 35.33707346],
        #                 [62.44655336, 66.74530732, 93.35023793]])
        
        # self.x.append(x)
        # self.y.append(y)
        # self.z.append(z)

        #self.sc = self.ax.scatter(self.X, self.Y, self.Z, marker="o", s=300, cmap=cm.Greys_r)
        #self.clr_mesh = self.ax.pcolormesh(x, y, z, shading='auto')
        self.fig.canvas.flush_events()
        plt.draw()
        self.fig.canvas.draw_idle()
        #plt.pause(0.1)
        
        

    def _close(self):
        plt.close(self.fig)


@macro
class LivePlot(object):
    """
    Start a live plot recorder which will plot coming scans. ::

        liveplot [<x>] <y>

    Examples::

        liveplot xmotor diode1
        liveplot diode1
    """
    def __init__(self, data1, data2=None):
        basename = 'plot'
        name = basename
        i = 2
        while name in [r.name for r in Recorder.getinstances()]:
            name = basename + '_%d' % i
            i += 1
        self.data1 = data1.name if hasattr(data1, 'name') else data1
        self.data2 = data2.name if hasattr(data2, 'name') else data2
        self.name = name
    def run(self):
        rec = PlotRecorder(data1=self.data1, data2=self.data2, name=self.name)
        rec.start()

@macro
class LivePlotMesh(object):
    """
    Start a live plot recorder which will plot coming scans. ::

        liveplot [<x>] <y>

    Examples::

        liveplot xmotor diode1
        liveplot diode1
    """
    def __init__(self, data1, data2, data_z):
        basename = 'plot'
        name = basename
        i = 2
        while name in [r.name for r in Recorder.getinstances()]:
            name = basename + '_%d' % i
            i += 1
        self.data1 = data1.name if hasattr(data1, 'name') else data1
        self.data2 = data2.name if hasattr(data2, 'name') else data2
        self.data_z = data_z.name if hasattr(data_z, 'name') else data_z
        self.name = name
    def run(self):
        rec = PlotRecorderMesh(data1=self.data1, data2=self.data2, data_z=self.data_z, name=self.name)
        rec.start()
