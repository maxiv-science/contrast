...
class LC400Waveform(object):
    ...
    def configure_recorder_json(self):
        """
        Create json to configure LC400 recorders:
           json = configure_recorder_json()
        """
        data = {}
        data["duration_seconds"] = self.totaltime

        scalefactor = self.clockcycledelay + 1 # The LC400 recorder is offset by 1 from the waveform clock
        data["clock_multiplier"] = scalefactor

        # check if latency between trigger pulses can be resolved by the LC400 recorder
        if self.latencytime < self.effectiveclockcycle:
            raise Warning(f"LC400 clock cycle is {self.effectiveclockcycle} s, given latency time is only {self.latencytime} s.")

        # configure the functions of the recorders
        data["tracks"] = {
            "1": {"address": "sensor_monitor_1"},
            "2": {"address": "sensor_monitor_2"},
            "3": {"address": "sensor_monitor_3"},
            "4": {"address": "ttl_input_pin_1"},
            "5": {"address": "wavetable_index_1"},
            "6": {"address": "wavetable_index_2"},
            "7": {"address": "wavetable_index_3"},
            "8": {"address": "ttl_output_pin_9"},
        }
...
class LC400SteppedWaveform(object):
    """
    Class to generate stepped waveform configs for the LC400 piezo controller.
    It generates a JSON string, that can be send to the LC400 Tango
    Server to configure the waveform.
    """
    ...
    def __init__(self, axis, startpoint, endpoint, scanpoints, exposuretime, latencytime, jitter):
        # waveform parameters
        self.axis = axis
        self.axisname = "axis%i" % axis
        stepsize = (endpoint-startpoint)/(scanpoints-1)
        ...
        # calculate total time per line (expusure time + latency time)
        self.totaltime = (self.exposuretime+self.latencytime)*self.scanpoints
        ...
        # define start cycle of linear phase.
        # This is the time when the LC 400 creates the trigger to start the pandabox for position recording and triggering of other detectors
        self.triggerstartindex = math.ceil(self.latencytime/self.effectiveclockcycle)
        print("triggger start index: ", self.triggerstartindex)
        # start point of the waveform in absolute coordinates of the scanner
        self.create_positions()
        self.absolutstartposition = self.positions[0]
        # print("start position of line is at:", self.absolutstartposition)
        print("scan points: ", self.scanpoints)
    ...
    def create_positions(self):
        ...
        positions = np.linspace(self.startpoint, self.endpoint, self.scanpoints)
        rel_jitter = np.random.uniform(low  = -.5*self.jitter,
                            high = .5*self.jitter,
                            size = np.shape(positions))
        self.positions = positions + rel_jitter
        self.absolutstartposition = self.positions[0]
        print("Absolute start position: ", self.absolutstartposition)
    ...
    def waveform(self):
        ...
        x = []
        n = 0
        pointtime = self.latencytime+self.exposuretime
        for t in self.time():
            if t  <= pointtime*(n+1):
                x.append(self.positions[n])
            else:
                print("not used: ", t)
            if t + self.effectiveclockcycle > pointtime*(n+1):
                n = n+1
    ...
    def json(self):
        ...
        triggers = {}
        triggers["count"] = 1
        triggers["on"] = [self.triggerstartindex]
        triggers["off"] = [self.triggerstartindex+1]
        # function definition of output pins
        # we use pin 9 (Out 4) to create teh start trigger
        output_pins = {}
        output_pins["9"] = {"polarity": 0, 
