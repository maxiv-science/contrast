...
class calc_ssa(object):
    ...
    %calc_ssa <endstation_str> <coherent_beam_bool> optional: <FZP_diameter_in_um>

    usage / examples:
        %calc_ssa  D False          # calculates the SSA setting for a high-flux beam
                                    # at the current energy at the diffraction endstation
        %calc_ssa  I True 75        # calculates the SSA setting for a fully coherent beam
                                    # at the current energy at the imaging endstation
                                    # for a FZP of 75um diameter
    """
    ...
    def input_validation(self):
        if not(self.endstation in self.accepted_names_D or self.endstation in self.accepted_names_I):
            print("    [ERROR] No correct endstation given.")
            print("            Use 'D' for the diffraction endstation.")
            print("            Use 'I' for the imaging endstation.")
            return False
        elif self.endstation in self.accepted_names_I and not(isinstance(self.FZP_diameter_um, (int, float))):
    ...
    def get_current_energy(self):
        ...
        self.wavelength_m = 12400. / self.photon_energy_eV *1.e-10
    ...
    def get_acceptance(self):
        if self.endstation in self.accepted_names_D:
            ...
            self.accepteance_m = np.array([1.e-6*self.FZP_diameter_um, 1.e-6*self.FZP_diameter_um]) 
    ...
    def print_result(self):
        ...
        print('-'*20)
        print('    required SSA opening:', "ssa_gapx {0:.2f}".format(self.ssa_opening_m[0]*1.e6), "ssa_gapy {0:.2f}".format(self.ssa_opening_m[1]*1.e6))
    ...
    def run(self):
        if self.input_validation():
            ...
            self.ssa_opening_m = self.wavelength_m*self.d_ssa_m/2./self.accepteance_m
