...
class Tomo(object):
    ...
    usage / examples:
        ...
        %tomo 20 assume_xz_circle=True
                            ...
                            movement for the z-position will hence be
                            caluclated as a cosine-movment according to the
                            sin-fit of the x-direction and z-value of the first
                            saved position. (for when the z-position at each
                            angle is diffcult / impossible to determine)
    ...
    def get_header(self):
        return ('# date-time            '
                + ' '.join([m.ljust(8) for m in self.motors]))
    ...
    def plot_fits(self):
        ...
        x0 = self.my_sin(0, *self.fitx[0])
        y0 = self.my_sin(0, *self.fity[0])
        z0 = self.my_sin(0, *self.fitz[0])
        ...
        plt.subplot(2, 4, 1)
        plt.scatter(x, z)
        plt.plot(self.my_sin(r_deg_plot, *self.fitx[0]),
                 self.my_sin(r_deg_plot, *self.fitz[0]), c='r', alpha=0.5)
