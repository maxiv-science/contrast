...
class Calc_OSA(object):
    """
    Calculate the maximum distance of the OSA from the sample and the
    corresponding size of the OSA. From there smaller and larger OSA
    diameters are possible if the OSA is placed closer to the sample.
    Those possibilities can be read from a created plot. If an OSA
    diameter / position is given, the positioning possibilities will be
    checked. Any problems with the positioning will be pointed out and
    solutions will be offered.

    %calc_osa <FZP_diameter_um> <FZP_drn_nm> <CS_diamter_um>
              <photon_energy_keV>
    ...
    usage / examples:
        %calc_osa 75 100 30 8          # calculates the OSA possibilites for a
                                       # Fresnel zone plate with 75 um
                                       # diameter, 100 nm outer most
                                       # zone width, a central stop of 30 um
                                       # diameter at 8 keV photon energy.

        %calc_osa 75 100 30 8 5 None   # ...
                                       # and will tell you where a 5 um
                                       # diameter OSA can be placed.

        %calc_osa 75 100 30 8 None 1   # ...
                                       # and will tell you what OSA sizes would
                                       # be possible for a placement 1 mm
                                       upstream from the focus.

        %calc_osa 75 100 30 8 5 1      # ...
                                       # and will check if a 5 um OSA at 1 mm
                                       # from the focus will do, and if not
                                       # tell you what you could change.
    """

    def __init__(self, FZP_diameter_um=75, FZP_drn_nm=100, CS_diameter_um=20,
                 photon_energy_keV=8, OSA_diameter_um=None,
    ...
    def plot_result(self):
        ...
        # plot FZP
        plt.plot([-1. * self.f1_mm, -1. * self.f1_mm],
                 [-0.5 * self.FZP_diameter_um, 0.5 * self.FZP_diameter_um],
                 c='k', lw=5)
    ...
    def calc_max_distance(self):
        ...
        x_intersect_um = ((rCS_um + rFZP_um) / ((rFZP_um / f1_um)
                          + (rCS_um / f2_um)))
    ...
    def run(self):
        if self.input_validation():
            ...
            self.photon_wavelength_m = 12.4 / self.photon_energy_keV * 1e-10
            self.f1_mm = (1e-12 * self.FZP_diameter_um * self.FZP_drn_nm
            ...
            if self.OSA_diameter_um is None and self.OSA_distance_mm is None:
                ...
                self.plot_result()
                  ...
                  and self.OSA_distance_mm is None):
                ...
                self.plot_result()
                  ...
                  and self.OSA_distance_mm is not None):
                # the user only has given a disance, ... so wants to know a
                # good size/region
                ...
                if self.OSA_diameter_um >= self.CS_diamter_um:
                    ...
                    if ((self.CS_diamter_um - self.d_OSA_max_um)
                            / self.f_OSA_max_mm
                        > (self.CS_diamter_um - self.OSA_diameter_um)
                            / self.OSA_distance_mm):
                        print('[WARNING] your OSA is letting some of the 2nd'
                              'order through!')
                        ...
                        self.print_suggested_diameters()
                        self.plot_result()
                    # checking if the given distance lets some of the 2nd order
                    # through
                    elif ((self.d_OSA_max_um / self.f_OSA_max_mm)
                          > (self.OSA_diameter_um / self.OSA_distance_mm)):
